...
def sync_spiders():
    ...
    for project in Project.query.all():
        ...
        SpiderInstance.update_spider_instances(project.project_id, spider_instance_list)
...
def run_spider_job(job_instance_id):
    ...
    try:
        ...
        app.logger.info('[run_spider_job][project:%s][spider_name:%s][job_instance_id:%s]' % (
            job_instance.project_id, job_instance.spider_name, job_instance.job_instance_id))
...
def reload_runnable_spider_job_execution():
    ...
    for job_instance in JobInstance.query.filter_by(enabled=0, run_type="periodic").all():
        job_id = "spider_job_%s:%s" % (job_instance.job_instance_id, int(time.mktime(job_instance.date_modified.timetuple())))
        ...
        if job_id not in running_job_ids:
            ...
            app.logger.info('[load_spider_job][project:%s][spider_name:%s][job_instance_id:%s][job_id:%s]' % (
                job_instance.project_id, job_instance.spider_name, job_instance.job_instance_id, job_id))
    # remove invalid jobs
