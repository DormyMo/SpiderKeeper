...
class SkCommand():
    ...
    def setup_config(self):
        opts, args = self._parse_opts(app.config)
        app.config.update(dict(
            ...
            SERVER_NAME="{}:{}".format(opts.host, opts.port)
        ))
        if opts.verbose:
            app.logger.setLevel(logging.DEBUG)
        initialize()
        app.logger.info("SpiderKeeper setup on http://%s:%s username:%s/password:%s with %s servers:%s" % (
